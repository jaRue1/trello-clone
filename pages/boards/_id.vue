<template>
  ...
  >
    ...
    <h1>{{ board.title }}</h1>
    <!-- utilizing plugin filter.js -->
    <small>created {{ board.dateCreated | formatDate }}</small>
    <div class="d-flex flex-row pr-6 pt-3">
      <!-- implementing drag and drop  &&  running a for loop that makes boards-->
       <div
...
export default {
  ...
  async asyncData({ params }) {
    // get our board data before page load, and then after that await changes
  ...
  created() {
    ...
    let boardRef = $nuxt.$fire.firestore
      ...
      .collection('boards')
      .doc(tempId)
      .onSnapshot((doc) => { // constantly checks to see if board has been updated
        if (doc.exists) {
          that.board = doc.data()
  ...
  methods: {
    ...
    async updateBoard() { // update board and save it to db
      let that = this
      await that.$fire.firestore
        .collection('users')
        .doc(that.$fire.auth.currentUser.uid)
        .collection('boards')
        .doc(that.board.id)
        .update(that.board, { merge: true }) // only update data that has been added
    }
  },
}
</script>
